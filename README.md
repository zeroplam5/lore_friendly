...
`design/DESIGN.md`의 `library-references` 프레임에는 원래 "🗑 자료 삭제" 버튼이 있었는데, 3차 개발 1차 구현 시 빠뜨렸던 것을 사용자가 실사용 중 발견해 알려줘서 수정했습니다. `useLibraryStore.removeReference(id)` 추가 + 좌측 목록 항목 호버 시 나타나는 휴지통 아이콘과 우측 메타 패널의 "자료 삭제" 버튼(원본 디자인 위치) 양쪽에서 삭제 가능하도록 구현. 삭제 후 다음 자료로 자동 선택 전환되고, 전부 삭제하면 `library-empty-state`로 정상 전환됨을 Playwright로 확인.

#### 집필 탭 챕터/장면 추가·삭제·순서 이동 + 챕터 이름 수정 (사용자 요청 추가 기능)

작품 구조 트리에는 챕터 추가만 있고 삭제·순서 변경·이름 수정이 없었고 장면 쪽은 그마저도 없었는데, 작가가 집필 중 구조를 직접 바꿀 수 있어야 한다는 요청으로 챕터와 장면 양쪽 다 추가했습니다.

- `stores/useDraftStore.ts` — `removeChapter`/`moveChapter`/`renameChapter`에 더해 `addScene`/`removeScene`/`moveScene`을 대칭적으로 구현(모두 해당 챕터 안에서만 배열을 조작하고 `order` 필드를 재정렬). 삭제 시 삭제된 항목이 선택돼 있었으면 같은 챕터의 남은 첫 장면 → 없으면 프로젝트에서 장면이 있는 첫 챕터로 자동 대체 선택
- `components/write/ChapterTree.tsx` — 챕터 행에 ✏️(이름 수정, 인라인 입력으로 전환) + ↑ ↓ 🗑, 장면 행에도 동일하게 ↑ ↓ 🗑, 챕터마다 "+ 장면 추가" 버튼. 모두 마우스 호버 시에만 노출, 첫/마지막 항목은 해당 방향 이동 버튼 비활성화
- Playwright로 장면 추가 → 위로 이동 → 삭제, 챕터 이름 변경까지 실제 클릭으로 재현하고 새로고침 후에도 유지되는지 확인. 이 과정에서 개발 서버가 반복된 `npm install/uninstall`(플레이라이트 검증용) 이후 HMR 캐시가 손상돼 클릭 이벤트가 조용히 무반응이 되는 현상을 발견 — 실제 코드 버그가 아니라 `.next` 캐시 삭제 후 서버 재시작으로 해결되는 개발 환경 이슈였음을 확인(같은 함수가 재시작 후에는 정상 동작)

#### 집필 자동저장 유실 버그 수정 (사용자 실사용 중 발견)

작가가 원고를 수정한 직후(자동저장 디바운스 500ms가 끝나기 전) 다른 탭으로 이동하거나 새로고침하면 방금 쓴 내용이 저장되지 않고 이전 내용으로 되돌아가는 버그가 있었습니다. 실제로 사용자가 장면 본문을 고쳐 시간대 모순 문장을 넣고 자료실에서 기준 설정을 추가한 뒤 검증했는데 오류가 잡히지 않는다는 리포트로 발견했습니다.

- 1차 원인: 장면 전환·다른 탭 이동으로 `DocumentEditor`가 언마운트될 때 디바운스 타이머가 아직 안 끝난 상태면 `setState` 클로저가 유실됨 → `pendingRef`에 미반영 값을 들고 있다가 장면 전환·언마운트 시 즉시 flush하도록 수정
- 2차 원인(사용자가 실제로 부딪힌 것): 브라우저 새로고침/탭 닫기는 리액트 언마운트 cleanup이 실행될 기회 없이 JS 실행 컨텍스트 자체가 끝나버려 1차 수정으로도 못 잡음 → `beforeunload`/`pagehide` 이벤트를 감지해 대기 중인 저장을 그 즉시 flush하도록 추가
- 검증: `/api/check`에 동일 시나리오(하워드 스타크 활동 시기 1940~1970년 커스텀 기준 설정 + 본문에 "2010년에" 삽입)를 직접 호출해 파이프라인 자체는 정상 탐지함을 먼저 확인 → 그다음 Playwright로 "입력 → (대기 없이) 자료실 이동 → 복귀 → 검증" 전체 흐름을 실제 브라우저로 재현해 수정 전엔 유실·수정 후엔 정상 탐지됨을 확인 → 마지막으로 "입력 → 대기 없이 즉시 새로고침"까지 재현해 2차 수정도 확인
